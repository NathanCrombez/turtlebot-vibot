...
class MoveItDemo:
    def __init__(self):
        ...
        table_size = [0.2, 0.9, 0.01]
        #box1_size = [0.1, 0.05, 0.05]                                                       commented for the same reasons as previously
        #box2_size = [0.05, 0.05, 0.15]

        # Set the target size [l, w, h]
        target_size = [0.018, 0.018, 0.018]		#[0.02, 0.005, 0.12]	             original object dimensions in meters
        ...
        target_pose.pose.position.z = table_ground + table_size[2] + target_size[2] -0.08 / 2.0    #0.4+0.01+0.018/2 aka table_ground + table_size[2] + target_size[2] / 2.0
        ...
        place_pose.pose.position.z = table_ground + table_size[2] + target_size[2] -0.08 / 2.0     #0.4+0.01+0.018/2 aka table_ground + table_size[2] + target_size[2] / 2.0
        ...
        if result == MoveItErrorCodes.SUCCESS:
            ...
            success = True           #False                              from here we are forcing the success cases by stting and making it always true it makes an error appear and the arm stays to 										 the middle position
            n_attempts = 0

            # Repeat until we succeed or run out of attempts
            while success and n_attempts <= max_place_attempts:          #while not has been removed   added = after operator <
